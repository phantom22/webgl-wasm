function createSphere(options?: { long_bands?: number; lat_bands?: number; radius?: number; }) {
    options = options || {};

    const long_bands = options.long_bands || 32,
          lat_bands = options.lat_bands || 32,
          radius = options.radius || 1,
          lat_step = Math.PI / lat_bands,
          long_step = 2 * Math.PI / long_bands,
          num_positions = long_bands * lat_bands * 4,
          num_indices = long_bands * lat_bands * 6,
          positions = new Float32Array(num_positions * 3),
          normals = new Float32Array(num_positions * 3),
          uvs = new Float32Array(num_positions * 2),
